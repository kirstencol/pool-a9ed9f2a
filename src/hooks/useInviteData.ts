...
export const useInviteData = (inviteId: string, userName?: string) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        console.log("useInviteData - Demo data initialized");
  ...
  // Data fetching function with useCallback to prevent recreating it on every render
  const fetchInviteData = useCallback(async () => {
    if (!initComplete || !inviteId) return;
  ...
  }, [inviteId, loadMeetingFromStorage, userName, initComplete]);

  // When initialization is complete and inviteId is available, fetch data
  useEffect(() => {
    if (initComplete && inviteId) {
      fetchInviteData();
    }
  }, [fetchInviteData, initComplete, inviteId]);
